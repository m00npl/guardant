...
import { StatusPageData } from '../types';
...
export const StatusHeader: React.FC<StatusHeaderProps> = ({ data, lastUpdated }) => {
  const getOverallStatus = () => {
    const services = data.services;
    if (services.length === 0) return { status: 'unknown', message: 'No services configured' };
  ...
  return (
    <div className="bg-white border-b border-gray-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Maintenance banner */}
        {data.maintenance.filter(m => m.status !== 'completed').length > 0 && (
          <div className="mt-6">
