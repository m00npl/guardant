...
const HistoryPage = () => {
    ...
    const { data, loading, error, refetch } = (0, useStatusPage_1.useStatusPage)();
    ...
    const getUptimeForPeriod = (service, period) => {
        switch (period) {
            case '24h':
    ...
    const getAvgResponseTimeForPeriod = (service, period) => {
        switch (period) {
            case '24h':
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex items-center space-x-4 mb-6">
            <react_router_dom_1.Link to="/" className="flex items-center px-3 py-2 text-gray-600 hover:text-gray-900 transition-colors">
          ...
          <div className="flex items-center space-x-1 p-1 bg-gray-100 rounded-lg w-fit">
            {periods.map((period) => (<button key={period.key} onClick={() => setSelectedPeriod(period.key)} className={`px-4 py-2 rounded-md text-sm font-medium transition-colors ${selectedPeriod === period.key
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="card p-6">
              <div className="flex items-center">
                <lucide_react_1.TrendingUp className="h-8 w-8 text-success-600 mr-4"/>
                <div>
                  <div className="text-2xl font-bold text-gray-900">
                    {services.length > 0
            ? (0, api_1.formatUptime)(services.reduce((acc, s) => acc + getUptimeForPeriod(s, selectedPeriod), 0) / services.length)
            : '0%'}
                  </div>
            ...
            <div className="card p-6">
              <div className="flex items-center">
                <lucide_react_1.Calendar className="h-8 w-8 text-blue-600 mr-4"/>
                <div>
                  <div className="text-2xl font-bold text-gray-900">
        ...
        <section className="mb-12">
          ...
          <div className="space-y-6">
            {services.map((service) => (<div key={service.id} className="card p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                  <div className="bg-gray-50 rounded-lg p-4 text-center">
                    <div className="text-2xl font-bold text-success-600">
                      {(0, api_1.formatUptime)(getUptimeForPeriod(service, selectedPeriod))}
                    </div>
                  ...
                  {service.type !== 'github' && getAvgResponseTimeForPeriod(service, selectedPeriod) && (<div className="bg-gray-50 rounded-lg p-4 text-center">
                ...
                {service.regions.length > 0 && (<div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                      {service.regions.map((region) => (<div key={region.id} className="bg-gray-50 rounded-lg p-3">
                          <div className="flex items-center justify-between">
                            ...
                            {region.responseTime && service.type !== 'github' && (<span className="text-sm text-gray-600">
        ...
        <section className="mb-12">
          ...
          {(() => {
            ...
            return (<div className="space-y-4">
                {periodIncidents.map((incident) => (<div key={incident.id} className="card p-6">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3 mb-2">
                          <span className={`px-2 py-1 rounded-full text-xs font-medium border ${incident.severity === 'critical' ? 'text-error-600 bg-error-50 border-error-200' :
                        incident.severity === 'major' ? 'text-warning-600 bg-warning-50 border-warning-200' :
                            'text-blue-600 bg-blue-50 border-blue-200'}`}>
                            {incident.severity.toUpperCase()}
                          </span>
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${incident.status === 'resolved' ? 'text-success-600 bg-success-50' :
