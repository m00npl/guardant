...
exports.LoadTestRunner = exports.LoadTestConfigs = void 0;
...
class LoadTestRunner {
    constructor(config, tracing) {
        this.logger = (0, logger_1.createLogger)("load-tests");
        this.scenarios = [];
        this.results = [];
        this.config = config;
    ...
    async runLoadTestScenario(scenario) {
        ...
        const { concurrentUsers, durationSeconds, rampUpSeconds, rampDownSeconds, targetRPS, } = scenario.config;
    ...
    calculateMetrics(results) {
        ...
        const successRate = totalRequests > 0 ? successful.length / totalRequests : 0;
    ...
    calculateMaxRPS(results) {
        // Group requests by second and find the maximum
        const requestsBySecond = new Map();
    ...
    evaluateScenario(scenario, metrics) {
    ...
    generateRecommendations(results) {
        ...
        const highResponseTimeScenarios = results.filter((r) => r.metrics.avgResponseTime > 2000);
        if (highResponseTimeScenarios.length > 0) {
            recommendations.push({
                ...
                suggestion: "Consider optimizing database queries, adding caching, or scaling infrastructure",
            });
        }
        // Analyze error rates
        const highErrorRateScenarios = results.filter((r) => r.metrics.errorRate > 0.1);
        if (highErrorRateScenarios.length > 0) {
            recommendations.push({
                ...
                suggestion: "Immediate investigation required. Check server resources, database connections, and error logs",
            });
        }
        ...
        if (lowRPSScenarios.length > 0) {
            recommendations.push({
                ...
                suggestion: "Consider horizontal scaling, load balancing, or performance optimization",
            });
        }
        ...
        if (passRate < 0.8) {
            recommendations.push({
                ...
                suggestion: "Review infrastructure capacity, database performance, and application bottlenecks",
            });
        }
...
function createLoadTestRunner(config, tracing) {
    return new LoadTestRunner(config, tracing);
}
//# sourceMappingURL=load-testing.js.map