...
platformRoutes.post('/nests/:nestId/details', async (c) => {
  try {
...
platformRoutes.post('/users/:userId/status', async (c) => {
  try {
    ...
    // Log the action
    const platformAdmin = getAuthUser(c);
...
platformRoutes.post('/logs', async (c) => {
  try {
    const body = await c.req.json();
...
platformRoutes.post('/subscriptions/:nestId/override', async (c) => {
  try {
    const nestId = c.req.param('nestId');
    const body = await c.req.json();
