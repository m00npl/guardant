...
import { DatabaseError, ErrorCategory, ErrorSeverity } from './error-handling';
...
export interface IndexColumn {
  ...
  nulls?: 'FIRST' | 'LAST';
  collation?: string;
  opclass?: string; // Operator class for specific data types
}
...
export class DatabaseIndexManager {
  ...
  private tableAnalysis = new Map<string, TableAnalysis>();
  private recommendedIndexes = new Map<string, IndexDefinition[]>();
  ...
  constructor(
    ...
    this.logger = createLogger(`${serviceName}-index-manager`);
  ...
  async analyzeDatabase(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Database analysis failed', error as Error);
      throw new DatabaseError('Failed to analyze database for indexing', {
  ...
  private async analyzeTableStatistics(): Promise<void> {
    ...
    try {
      ...
      this.logger.error('Failed to analyze table statistics', error as Error);
    }
  }
    ...
    try {
      ...
      for (const row of result.rows) {
        ...
        const effectivenessScore = this.calculateEffectivenessScore(
          ...
          parseInt(row.tuples_fetched) || 0,
          indexSize
        );

        const usage: IndexUsageStats = {
  ...
  private calculateMaintenanceCost(indexSize: number, writeOperations: number): number {
  ...
  private async generateIndexRecommendations(): Promise<void> {
    ...
    for (const [tableGroup, indexes] of Object.entries(IndexStrategies)) {
  ...
  private calculateRecommendationScore(index: IndexDefinition, analysis: TableAnalysis): number {
  ...
  private async generateQueryBasedRecommendations(): Promise<void> {
    ...
    try {
      ...
      for (const row of result.rows) {
        ...
        for (const rec of recommendations) {
          ...
          const similar = existing.find(idx => 
            idx.columns.some(col => rec.columns.some(recCol => recCol.name === col.name))
          );
      ...
      this.logger.debug('Could not analyze pg_stat_statements', { error: (error as Error).message });
    }
  }

  private analyzeQueryForIndexes(query: string): IndexDefinition[] {
  ...
  async createIndex(indexDef: IndexDefinition): Promise<boolean> {
    try {
      ...
      this.logger.error('Failed to create database index', error as Error, {
        indexName: indexDef.name,
        table: indexDef.table
      });
      return false;
  ...
  async dropIndex(indexName: string): Promise<boolean> {
    try {
      ...
      this.logger.error('Failed to drop database index', error as Error, { indexName });
      return false;
    }
  }

  async reindexTable(tableName: string): Promise<boolean> {
    try {
      ...
      this.logger.error('Failed to reindex table', error as Error, { tableName });
      return false;
    }
  }
...
// Factory function
export function createDatabaseIndexManager(
  serviceName: string,
  dbConnection: any,
  tracing?: GuardAntTracing
