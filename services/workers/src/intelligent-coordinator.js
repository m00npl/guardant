...
exports.intelligentCoordinator = exports.IntelligentCoordinator = void 0;
...
class IntelligentCoordinator {
    ...
    async getAvailableRegions() {
        ...
        const activeRegions = new Set(activeWorkerAnts.map(ant => this.mapWorkerAntToRegion(ant)));
     ...
     */
    async routeMonitoring(nest, service) {
        ...
        if (service.monitoring?.minRegions) {
            ...
            if (targetRegions.length < minRegions) {
                ...
                const additionalRegions = await this.getAdditionalRegions(service, targetRegions, minRegions - targetRegions.length);
     ...
     */
    async queueRegionalMonitoring(nest, service, regionId) {
        ...
        await this.monitoringQueue.add(`check-${service.id}-${regionId}`, jobData, jobOptions);
     ...
     */
    async getAllSelectedRegions(service, preferredRegions) {
        const availableWorkerAnts = await this.getActiveWorkerAnts();
        const availableRegions = new Set(availableWorkerAnts.map(ant => this.mapWorkerAntToRegion(ant)));
        return preferredRegions.filter(regionId => availableRegions.has(regionId));
    }
    /**
     * Get closest regions to the target service
     */
    async getClosestRegions(service, preferredRegions) {
        ...
        const regionsWithDistance = regions
            ...
            if (!region)
                return null;
            const distance = this.calculateDistance(targetLocation, region.coordinates);
     ...
     */
    async getRoundRobinRegions(service, preferredRegions) {
        const available = await this.getAllSelectedRegions(service, preferredRegions);
     ...
     */
    async getFailoverRegions(service, preferredRegions) {
     ...
     */
    async getAdditionalRegions(service, existingRegions, needed) {
     ...
     */
    async findBestWorkerAntInRegion(service, regionId) {
        const activeWorkerAnts = await this.getActiveWorkerAnts();
        const regionWorkerAnts = activeWorkerAnts.filter(ant => this.mapWorkerAntToRegion(ant) === regionId &&
            ant.capabilities.serviceTypes.includes(service.type));
        if (regionWorkerAnts.length === 0)
            return null;
     ...
     */
    calculateWorkerAntScore(workerAnt) {
        let score = 0;
     ...
     */
    mapWorkerAntToRegion(workerAnt) {
        ...
        if (continent === 'North America') {
            if (country === 'Canada')
                return 'ca-central-1';
            if (city.includes('San Francisco') || city.includes('California'))
                return 'us-west-1';
     ...
     */
    async hasAvailableWorkerAntInRegion(regionId) {
        const activeWorkerAnts = await this.getActiveWorkerAnts();
    ...
    async getActiveWorkerAnts() {
        ...
        for (const key of keys) {
            try {
                ...
                if (data) {
                    const workerAnt = JSON.parse(data);
     ...
     */
    async estimateServiceLocation(target) {
...
exports.IntelligentCoordinator = IntelligentCoordinator;
// Parse Redis URL if provided
function getRedisConfigForIntelligentCoordinator() {
...
// Export singleton
exports.intelligentCoordinator = new IntelligentCoordinator(getRedisConfigForIntelligentCoordinator());
//# sourceMappingURL=intelligent-coordinator.js.map