...
import { createGuardAntTracing } from './tracing';
import { createGolemAdapter, DataType, GolemUtils } from './golem-adapter';
import { createUsageAnalyticsManager } from './usage-analytics';
import { createSLAReportingManager } from './sla-reporting';
import { createCostTrackingManager } from './cost-tracking';
import { createFailoverSystemManager } from './failover-system';

async function demonstrateGuardAntSystem() {
  const logger = createLogger('guardant-demo');
  
  logger.info('🚀 Initializing GuardAnt Multi-Tenant Monitoring System');

  // 1. Initialize OpenTelemetry tracing
  const tracing = createGuardAntTracing('guardant-demo', {
    enabled: true,
  ...
  const analyticsManager = createUsageAnalyticsManager(
    'guardant-analytics',
    {
      enabled: true,
  ...
  const slaManager = createSLAReportingManager(
    'guardant-sla',
    {
      enabled: true,
      defaultTargets: {
  ...
  const costManager = createCostTrackingManager(
    'guardant-cost',
    {
      enabled: true,
      currency: 'USD',
  ...
  const failoverManager = createFailoverSystemManager(
    'guardant-failover',
    {
      enabled: true,
  ...
  try {
    ...
    const configEntityKey = await golemAdapter.storeNestData(
      demoNestId,
      DataType.NEST_CONFIGURATION,
      nestConfig,
      { key: 'main-config', version: '1.0' }
    );
    ...
    await analyticsManager.trackPageView(
      ...
      'demo-user-1',
      'session-123',
      { userAgent: 'GuardAnt-Demo/1.0', referrer: '/' }
    );

    await analyticsManager.trackApiCall(
      demoNestId,
      '/api/services',
      'GET',
      200,
    ...
    const costSummary = await costManager.calculateCostSummary(
      demoNestId,
      startOfMonth,
      now
    );
    ...
    const retrievedConfig = await golemAdapter.retrieveNestData(
      demoNestId,
      DataType.NEST_CONFIGURATION,
      'main-config'
    );
...
export { demonstrateGuardAntSystem };