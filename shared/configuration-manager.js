...
exports.ConfigurationManager = exports.DefaultConfigurationSource = exports.FileConfigurationSource = exports.EnvironmentVariableSource = exports.ConfigurationSchemas = exports.Environment = exports.ConfigFieldType = void 0;
...
(function (ConfigFieldType) {
    ...
    ConfigFieldType["EMAIL"] = "email";
    ConfigFieldType["PORT"] = "port";
    ConfigFieldType["SECRET"] = "secret";
    ConfigFieldType["ENUM"] = "enum";
})(ConfigFieldType || (exports.ConfigFieldType = ConfigFieldType = {}));
var Environment;
...
class FileConfigurationSource {
    constructor(filePath, priority = 2) {
    ...
    async load() {
        try {
            ...
            throw new error_handling_1.ConfigurationError(`Failed to load config file: ${this.filePath}`, {
    ...
    watch(callback) {
        ...
        const fs = require('fs');
        const watcher = fs.watch(this.filePath, (eventType) => {
            if (eventType === 'change') {
...
exports.DefaultConfigurationSource = DefaultConfigurationSource;
class ConfigurationManager {
    ...
    async load() {
        ...
        try {
            ...
            throw new error_handling_1.ConfigurationError('Failed to load configuration', {
        ...
        for (const [key, fieldSchema] of Object.entries(this.schema)) {
            ...
            if (value !== undefined && fieldSchema.transform) {
                try {
                    ...
                    this.logger.warn(`Failed to transform config value for ${key}`, error);
                }
            }
            // Type conversion
            if (value !== undefined) {
    ...
    async validate() {
        ...
        if (this.validationErrors.length > 0) {
            this.logger.error('Configuration validation failed', new error_handling_1.ConfigurationError('Invalid configuration', {
                service: this.serviceName,
            ...
            if (this.getEnvironment() === Environment.PRODUCTION) {
                throw new error_handling_1.ConfigurationError(`Configuration validation failed: ${this.validationErrors.join(', ')}`, {
                    service: this.serviceName,
                    operation: 'configuration_validation'
                });
            }
    ...
    handleSourceChange(sourceName, changes) {
    ...
    notifyChange(key, oldValue, newValue, source) {
        const event = {
            key,
            oldValue,
            newValue,
        ...
        for (const listener of this.changeListeners) {
            try {
                ...
                this.logger.warn('Configuration change listener failed', error, { key });
            }
        }
        if (this.tracing) {
...
// Factory function
function createConfigurationManager(serviceName, tracing) {
    return new ConfigurationManager(serviceName, tracing);
}
// Utility function to create a fully configured manager
async function createServiceConfiguration(serviceName, configSchemas = ['SERVICE_BASE'], configFiles = [], tracing) {
    const manager = createConfigurationManager(serviceName, tracing);
    // Add schemas
    for (const schemaName of configSchemas) {
        if (schemaName in exports.ConfigurationSchemas) {
            manager.addSchema(schemaName, exports.ConfigurationSchemas[schemaName]);
        }
    }
