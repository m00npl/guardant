...
const Workers = () => {
    ...
    const [pendingWorkers, setPendingWorkers] = (0, react_1.useState)([]);
    const [approvedWorkers, setApprovedWorkers] = (0, react_1.useState)([]);
    ...
    const approveWorker = async (workerId, region = 'auto') => {
        try {
    ...
    if (loading) {
        ...
        <lucide_react_1.RefreshCw className="h-8 w-8 text-primary-600 animate-spin"/>
      </div>);
    }
    return (<div className="space-y-8">
      ...
      {error && (<div className="bg-error-50 border border-error-200 text-error-700 px-4 py-3 rounded">
          {error}
        </div>)}
      ...
      <div className="card">
        ...
        {pendingWorkers.length === 0 ? (<div className="p-12 text-center">
            <lucide_react_1.Users className="h-12 w-12 text-gray-300 mx-auto mb-4"/>
            <p className="text-gray-500">No pending worker registrations</p>
          </div>) : (<div className="divide-y divide-gray-200">
            {pendingWorkers.map(worker => (<div key={worker.workerId} className="p-6">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex space-x-2">
                    <button onClick={() => approveWorker(worker.workerId)} className="btn-primary btn-sm flex items-center">
                      <lucide_react_1.UserCheck className="h-4 w-4 mr-1"/>
                      Approve
                    </button>
                    <button onClick={() => rejectWorker(worker.workerId)} className="btn-secondary btn-sm flex items-center">
      ...
      <div className="card">
        ...
        {approvedWorkers.length === 0 ? (<div className="p-12 text-center">
            <lucide_react_1.Users className="h-12 w-12 text-gray-300 mx-auto mb-4"/>
                ...
                return (<tr key={worker.workerId}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div className="flex items-center">
                          <lucide_react_1.MapPin className="h-4 w-4 mr-1 text-gray-400"/>
                          {worker.region || 'auto'}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        <div className="flex items-center">
                          <lucide_react_1.Zap className="h-4 w-4 mr-1 text-yellow-500"/>
                          {worker.points || 0}
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                        {worker.lastHeartbeat ? (<div className="flex items-center">
                            <lucide_react_1.Clock className="h-4 w-4 mr-1 text-gray-400"/>
                            {getTimeSince(worker.lastHeartbeat)}
                          </div>) : ('Never')}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${isOnline
