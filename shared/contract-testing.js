...
exports.ContractTestRunner = exports.ContractTestConfigs = void 0;
exports.createContractTestRunner = createContractTestRunner;
/**
...
class ContractTestRunner {
    constructor(config, tracing) {
        this.logger = (0, logger_1.createLogger)("contract-tests");
        this.tests = [];
        this.config = config;
        this.tracing = tracing;
        this.initializeContractTests();
    }
    initializeContractTests() {
        // Admin API Contract Tests
        for (const [contractName, contract] of Object.entries(this.config.contracts)) {
    ...
    async runAllContractTests() {
        ...
        const report = {
            summary: {
                ...
                failed,
                skipped,
                breakingChanges: compatibilityIssues.filter((i) => i.type === "BREAKING").length,
                nonBreakingChanges: compatibilityIssues.filter((i) => i.type === "NON_BREAKING").length,
                deprecations: compatibilityIssues.filter((i) => i.type === "DEPRECATION").length,
            },
            tests: testResults,
            compatibilityIssues,
            recommendations,
    ...
    async testContract(contractName, endpoint) {
        ...
        try {
            ...
            // Make request
            const response = await this.makeRequest(serviceUrl + endpoint.path, endpoint.method, testData);
            // Validate response
            const responseValidation = this.validateResponse(response, endpoint.responseSchema);
            // Check for compatibility issues
            const compatibilityIssues = this.detectCompatibilityIssues(response, endpoint.responseSchema);
            return {
                testName,
                ...
                compatibilityIssues: [
                    {
                        ...
                        suggestion: "Check service availability and endpoint implementation",
                    },
                ],
                timestamp: Date.now(),
            };
        ...
        if (schema.type === "string") {
            if (typeof data === "string") {
                ...
                if (schema.enum && !schema.enum.includes(data)) {
                    errors.push(`String value '${data}' not in enum ${schema.enum.join(", ")}`);
                }
            }
        }
        // Number validation
    ...
    detectCompatibilityIssues(data, schema) {
        const issues = [];
        if (!schema)
            return issues;
    ...
    collectCompatibilityIssues(results) {
        const issues = [];
    ...
    generateContractRecommendations(results) {
        ...
        const breakingChanges = results.filter((r) => r.compatibilityIssues?.some((i) => i.type === "BREAKING"));
        ...
        const validationIssues = results.filter((r) => r.responseValidation && !r.responseValidation.valid);
...
function createContractTestRunner(config, tracing) {
    return new ContractTestRunner(config, tracing);
}
//# sourceMappingURL=contract-testing.js.map