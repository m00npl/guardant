...
exports.RequestValidator = exports.ValidationSchemas = exports.FieldType = void 0;
...
(function (FieldType) {
    ...
    FieldType["SUBDOMAIN"] = "subdomain";
    FieldType["TRANSACTION_HASH"] = "transaction_hash";
    FieldType["ETHEREUM_ADDRESS"] = "ethereum_address";
    FieldType["SERVICE_TYPE"] = "service_type";
    FieldType["JSON"] = "json";
...
class RequestValidator {
    constructor(serviceName, tracing) {
        ...
        this.logger = (0, logger_1.createLogger)(`${serviceName}-validator`);
        this.tracing = tracing;
        this.sanitizer = (0, sanitization_1.createInputSanitizer)(serviceName, tracing);
    }
    validateField(value, rule, context) {
    ...
    async validateRequest(data, schema, context) {
        const errors = [];
        const warnings = [];
        ...
        try {
            ...
            const sanitizationResult = this.sanitizer.sanitize(data, sanitization_1.SanitizationConfigs.STANDARD);
            ...
            if (errors.length === 0 && schema.businessRules) {
                for (const businessRule of schema.businessRules) {
                    try {
                        ...
                        this.logger.error(`Business rule '${businessRule.name}' execution failed`, error, {
            ...
            if (!valid) {
                this.logger.warn('Request validation failed', new error_handling_1.ValidationError('Request validation failed', {
            ...
            this.logger.error('Request validation error', error, { schema: schema.name, context });
            return {
                valid: false,
                errors: [{
                        valid: false,
    ...
    async validateNestRegistration(data, context) {
        return this.validateRequest(data, exports.ValidationSchemas.NEST_REGISTRATION, context);
    }
    async validateServiceCreation(data, context) {
        return this.validateRequest(data, exports.ValidationSchemas.SERVICE_CREATION, context);
    }
    async validateUserAuth(data, context) {
        return this.validateRequest(data, exports.ValidationSchemas.USER_AUTH, context);
    }
    async validateServiceUpdate(data, context) {
        return this.validateRequest(data, exports.ValidationSchemas.SERVICE_UPDATE, context);
    }
    async validatePaymentTransaction(data, context) {
        return this.validateRequest(data, exports.ValidationSchemas.PAYMENT_TRANSACTION, context);
    }
}
exports.RequestValidator = RequestValidator;
// Middleware factory for HTTP frameworks
function createValidationMiddleware(validator, schemaName) {
    return async (c, next) => {
        ...
        try {
            ...
            // Get schema
            const schema = typeof schemaName === 'string' ? exports.ValidationSchemas[schemaName] : schemaName;
            if (!schema) {
                return c.json({
...
// Factory function
function createRequestValidator(serviceName, tracing) {
