...
const heartbeat_listener_1 = require("./heartbeat-listener");
const logger = (0, logger_1.createLogger)('monitoring-scheduler');
// Configuration
const config = {
...
// Scheduled services
const scheduledServices = new Map();
let schedulerInterval = null;
// URL deduplication
const urlToServices = new Map(); // URL -> Set of service IDs
const pendingChecks = new Map(); // URL -> timestamp of last check
...
async function connectToRabbitMQ() {
    rabbitmqConnection = await amqplib_1.default.connect(config.rabbitmqUrl);
...
async function sendCheckCommand(service) {
    if (!rabbitmqChannel)
        return;
    ...
    if (regions.length > 0) {
        ...
        for (const region of regions) {
            await rabbitmqChannel.publish('worker_commands', `check_service_once.${region}`, Buffer.from(JSON.stringify(command)), { persistent: true });
        ...
        await rabbitmqChannel.publish('worker_commands', 'check_service_once', Buffer.from(JSON.stringify(command)), { persistent: true });
...
async function propagateResult(result, serviceId) {
...
async function listenForResults() {
    ...
    await rabbitmqChannel.consume(q.queue, async (msg) => {
        ...
        try {
            ...
            if (result.cacheKey) {
                await redis.setex(`check:cache:${result.cacheKey}`, CHECK_CACHE_TTL / 1000, // TTL in seconds
                JSON.stringify({
                    status: result.status,
...
async function updateServiceStats(service, result) {
    // Update statistics
    service.stats.checksCompleted++;
    globalStats.totalChecksCompleted++;
    if (result.status === 'up') {
...
async function start() {
    try {
        ...
        await (0, heartbeat_listener_1.startHeartbeatListener)(rabbitmqChannel);
