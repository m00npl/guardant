...
class CacheManager {
    constructor() {
        ...
        this.syncLog = [];
        this.maxSyncLogEntries = 100; // Keep last 100 sync attempts
        ...
        const defaultCachePath = path.join(xdg.cache(), "guardant-cache", "cache.json");
    ...
    loadCache() {
        try {
            if (fs.existsSync(this.cachePath)) {
                ...
                console.log(`💾 Loaded ${this.cache.size} cache entries from ${this.cachePath}`);
                if (invalidEntries > 0) {
    ...
    async syncWithGolemBase(golemStorage) {
        ...
        try {
            ...
            for (let i = 0; i < unsyncedEntries.length; i += batchSize) {
                ...
                for (const entry of batch) {
                    try {
                        ...
                        const entityKey = await golemStorage.storeData(entry.key, entry.data, 3600 // 1 hour TTL
                        );
                        if (entityKey) {
            ...
            console.log(`✅ Sync completed: ${synced} synced, ${failed} failed in ${syncEntry.duration}ms`);
        }
        catch (error) {
