...
test_1.test.describe('Full User Journey', () => {
    (0, test_1.test)('Complete flow: Registration → Service Creation → Status Viewing', async ({ page, context }) => {
        // Step 1: Register new nest
        await test_1.test.step('Register new nest', async () => {
        ...
        await test_1.test.step('Create monitoring service', async () => {
            ...
            await (0, test_1.expect)(page.locator(`text=${TEST_SERVICE.name}`)).toBeVisible();
        });
        // Step 3: Wait for monitoring data
        await test_1.test.step('Verify monitoring is working', async () => {
            ...
            await (0, test_1.expect)(statusIndicator).toHaveClass(/status-up/, { timeout: 40000 });
            // Click on service to see details
            await page.click(`text=${TEST_SERVICE.name}`);
            // Should show response time chart
            await (0, test_1.expect)(page.locator('.response-time-chart')).toBeVisible();
            await (0, test_1.expect)(page.locator('.uptime-percentage')).toContainText('%');
        });
        // Step 4: View public status page
        await test_1.test.step('View public status page', async () => {
            ...
            await (0, test_1.expect)(publicPage.locator('h1')).toContainText('Service Status');
            await (0, test_1.expect)(publicPage.locator(`text=${TEST_SERVICE.name}`)).toBeVisible();
            // Status should be operational
            const publicStatus = publicPage.locator(`[data-service="${TEST_SERVICE.name}"] .status`);
            await (0, test_1.expect)(publicStatus).toContainText('Operational');
            await publicPage.close();
        });
        // Step 5: Test embed widget
        await test_1.test.step('Test embed widget', async () => {
            // Get embed code
            ...
            (0, test_1.expect)(embedCode).toContain(`nest=${TEST_NEST.subdomain}`);
      ...
      `);
            ...
            await (0, test_1.expect)(widgetStatus).toBeVisible();
            await embedPage.close();
        });
        // Step 6: Test user management
        await test_1.test.step('Add team member', async () => {
            ...
            await (0, test_1.expect)(page.locator(`text=${inviteEmail}`)).toBeVisible();
        });
        // Step 7: Test settings and subscription
        await test_1.test.step('Update nest settings', async () => {
            await page.goto('/dashboard/settings');
            // Update display name
            await page.fill('input[name="displayName"]', 'E2E Test Nest');
            await page.click('button:has-text("Save Settings")');
            // Should show success message
            await (0, test_1.expect)(page.locator('.toast-success')).toContainText('Settings updated');
            // Check subscription status
            await page.click('text=Subscription');
            await (0, test_1.expect)(page.locator('.subscription-tier')).toContainText('Free');
        });
    });
    (0, test_1.test)('Multi-region monitoring', async ({ page }) => {
        ...
        await (0, test_1.expect)(page.locator('.region-selector')).toBeVisible();
        await (0, test_1.expect)(page.locator('text=us-east-1')).toBeVisible();
        await (0, test_1.expect)(page.locator('text=eu-west-1')).toBeVisible();
        await (0, test_1.expect)(page.locator('text=ap-southeast-1')).toBeVisible();
    });
    (0, test_1.test)('Incident management', async ({ page }) => {
        await page.goto('/login');
        await page.fill('input[name="email"]', TEST_NEST.email);
        await page.fill('input[name="password"]', TEST_NEST.password);
        await page.click('button[type="submit"]');
        ...
        await (0, test_1.expect)(page.locator('text=E2E Test Incident')).toBeVisible();
        // Update incident
        ...
        await (0, test_1.expect)(page.locator('.incident-status')).toContainText('Resolved');
    });
    (0, test_1.test)('Performance: Dashboard load time', async ({ page }) => {
        await page.goto('/login');
        await page.fill('input[name="email"]', TEST_NEST.email);
        await page.fill('input[name="password"]', TEST_NEST.password);
        await page.click('button[type="submit"]');
        ...
        await (0, test_1.expect)(page.locator('.services-overview')).toBeVisible();
        await (0, test_1.expect)(page.locator('.uptime-summary')).toBeVisible();
        await (0, test_1.expect)(page.locator('.recent-incidents')).toBeVisible();
    });
    (0, test_1.test)('Mobile responsiveness', async ({ page }) => {
        // Set mobile viewport
        await page.setViewportSize({ width: 375, height: 667 });
        await page.goto('/');
        // Mobile menu should be visible
        await (0, test_1.expect)(page.locator('.mobile-menu-button')).toBeVisible();
        // Click mobile menu
        await page.click('.mobile-menu-button');
        // Navigation should be visible
        await (0, test_1.expect)(page.locator('nav.mobile-nav')).toBeVisible();
        ...
        await (0, test_1.expect)(page.locator('.mobile-dashboard')).toBeVisible();
    });
});
//# sourceMappingURL=full-user-journey.spec.js.map