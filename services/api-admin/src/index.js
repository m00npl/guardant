...
const health_1 = require("/app/shared/health");
const deployment_api_1 = require("./deployment-api");
const workers_1 = require("./routes/workers");
const metrics_1 = require("/app/shared/metrics");
const tracing_1 = require("/app/shared/tracing");
const error_handling_1 = require("/app/shared/error-handling");
const retry_1 = require("/app/shared/retry");
const circuit_breaker_1 = require("/app/shared/circuit-breaker");
const dead_letter_queue_1 = require("/app/shared/dead-letter-queue");
const index_1 = require("/app/packages/golem-base-l3/src/index");
const config_manager_1 = require("/app/shared/config-manager");
const index_2 = require("/app/packages/auth-system/src/index");
const platform_routes_simple_1 = require("./platform-routes-simple");
const index_3 = require("/app/packages/payments/src/index");
// Initialize service logger
const logger = (0, logger_1.createLogger)('api-admin');
...
let redisWithMetrics = {
    async get(key) {
        return redisCircuitBreaker.execute(async () => {
            const result = await retryManager.retryRedisOperation(() => redis.get(key), `redis_get_${key}`);
            if (!result.success) {
                throw result.error || new error_handling_1.DatabaseError('get', { service: 'api-admin', operation: 'redis_get' });
            }
    ...
    async set(key, value) {
        const result = await redisCircuitBreaker.execute(async () => {
            const result = await retryManager.retryRedisOperation(() => redis.set(key, value), `redis_set_${key}`);
            if (!result.success) {
                throw result.error || new error_handling_1.DatabaseError('set', { service: 'api-admin', operation: 'redis_set' });
            }
    ...
    async setex(key, seconds, value) {
        const result = await redisCircuitBreaker.execute(async () => {
            const result = await retryManager.retryRedisOperation(() => redis.setex(key, seconds, value), `redis_setex_${key}`);
            if (!result.success) {
                throw result.error || new error_handling_1.DatabaseError('setex', { service: 'api-admin', operation: 'redis_setex' });
            }
    ...
    async del(key) {
        const result = await redisCircuitBreaker.execute(async () => {
            const result = await retryManager.retryRedisOperation(() => redis.del(key), `redis_del_${key}`);
            if (!result.success) {
                throw result.error || new error_handling_1.DatabaseError('del', { service: 'api-admin', operation: 'redis_del' });
            }
    ...
    async ping() {
        ...
        if (!result.success) {
            throw result.error || new error_handling_1.DatabaseError('ping', { service: 'api-admin', operation: 'redis_ping' });
        }
        return result.data;
    },
    async keys(pattern) {
        ...
        if (!result.success) {
            throw result.error || new error_handling_1.DatabaseError('keys', { service: 'api-admin', operation: 'redis_keys' });
        }
        return result.data;
    },
...
const keys = {
    ...
    nestByEmail: (email) => `nest:email:${email}`,
    nestPassword: (nestId) => `nest:password:${nestId}`,
    services: (nestId) => `services:${nestId}`,
...
const hybridStorage = {
    async createNest(nest) {
        ...
        try {
            await index_1.golemStorage.createNest(nest);
    ...
    async storeNestPassword(nestId, passwordHash) {
        await redis.set(keys.nestPassword(nestId), passwordHash);
    },
    async getNestPassword(nestId) {
    ...
    async updateService(nestId, serviceId, updates) {
    ...
    async deleteService(nestId, serviceId) {
        const services = await this.getServicesByNest(nestId);
...
const rabbitmqService = {
    async connectToRabbitMQ() {
        const timer = new logger_1.PerformanceTimer(logger, 'RabbitMQ connection', { component: 'rabbitmq' });
        try {
            logger.info('Connecting to RabbitMQ', {
                component: 'rabbitmq',
                url: rabbitmqConfig.url
            });
            rabbitmqConnection = await amqplib_1.default.connect(rabbitmqConfig.url);
            ...
            logger.error('RabbitMQ connection failed', error, { component: 'rabbitmq' });
            throw error;
        }
    },
    async startWorkerResultsConsumer() {
        ...
        await rabbitmqChannel.consume(rabbitmqConfig.workerResultsQueue, async (message) => {
            ...
            try {
                ...
                const duration = performance.now() - startTime;
                logger.error('Error processing worker result', error, {
                    component: 'rabbitmq',
                    duration
                });
    ...
    async processWorkerResult(result) {
        ...
        if (currentStatus) {
            ...
            const regionIndex = serviceStatus.regions.findIndex((r) => r.id === regionId);
            if (regionIndex !== -1) {
    ...
    async sendWorkerCommand(command, data) {
        ...
        await rabbitmqChannel.publish(rabbitmqConfig.workerCommandsExchange, command, Buffer.from(message), { persistent: true });
    ...
    async requestServiceMonitoring(service) {
        try {
    ...
    async stopServiceMonitoring(nestId, serviceId) {
        try {
...
let rabbitmqCircuitBreaker;
let golemCircuitBreaker;
let ethereumCircuitBreaker;
...
// Helper functions
const extractNestId = (c) => {
    try {
        const user = (0, index_2.getAuthUser)(c);
...
let healthEndpoints = {
    basic: (c) => c.text('OK', 200),
    detailed: (c) => c.json({ status: 'initializing' }, 503),
    ready: (c) => c.text('Not Ready', 503),
    live: (c) => c.text('OK', 200)
};
...
function registerApiEndpoints(app) {
        ...
        return tracing.traceBusinessEvent('user_registration', async (span) => {
            try {
                ...
                if (!name || !subdomain || !email || !password) {
                    throw new error_handling_1.ValidationError('Missing required fields: name, subdomain, email, password', {
                        service: 'api-admin',
                ...
                if (existingNest) {
                    ...
                    throw new error_handling_1.BusinessLogicError('Subdomain is already taken', {
                        service: 'api-admin',
                        operation: 'user_registration',
                        requestId: c.get('requestId'),
                        metadata: { subdomain }
                ...
                try {
                    const subscriptionResult = await tracing.tracePayment('subscription_creation', '0', 'eth', async (paymentSpan) => {
                        ...
                        return await paymentManager.createSubscription(nest.id, 'free', 'eth', false);
                    });
                    ...
                    span.addEvent('subscription_creation_error', { error: error.message });
                    requestLogger.error('Failed to create subscription', error, { nestId: nest.id });
                }
    ...
    app.post('/api/admin/subscription/plans', async (c) => {
        try {
            const plans = (0, index_3.getAllPlans)();
            return c.json({
                success: true,
                data: plans
            });
    ...
    app.post('/api/admin/nest/profile', async (c) => {
        try {
            ...
            if (!nest) {
                return c.json({ success: false, error: 'Nest not found' }, 404);
            }
            return c.json({
                success: true,
                data: nest
    ...
    app.post('/api/admin/services/create', async (c) => {
        ...
        return tracing.traceBusinessEvent('service_creation', async (span) => {
            ...
            try {
                ...
                // Validate regions
                const invalidRegions = serviceData.monitoring.regions.filter(regionId => !availableRegions.find(r => r.id === regionId && r.available));
                ...
                try {
                    ...
                    span.addEvent('subscription_usage_update_failed', { error: error.message });
    ...
    app.post('/api/admin/subscription/create', async (c) => {
        try {
            ...
            const result = await paymentManager.createSubscription(nestId, planId, paymentMethod || 'eth', isYearly || false, walletAddress);
            if (!result.success) {
    ...
    app.post('/api/admin/subscription/upgrade', async (c) => {
        try {
            ...
            const result = await paymentManager.upgradeSubscription(nestId, planId, isYearly || false, body.walletAddress);
            if (!result.success) {
                return c.json({ success: false, error: result.error }, 400);
            }
            return c.json({
    ...
    app.post('/api/admin/subscription/cancel', async (c) => {
        try {
            ...
            const result = await paymentManager.cancelSubscription(nestId, immediately || false);
            if (!result.success) {
                return c.json({ success: false, error: result.error }, 400);
            }
            return c.json({
    ...
    app.post('/api/admin/wallet/connect', async (c) => {
        try {
            ...
            if (!walletType) {
                return c.json({ success: false, error: 'Wallet type required' }, 400);
            }
            const walletInfo = await paymentManager.connectWallet(walletType);
            return c.json({
                success: true,
                data: walletInfo
            });
...
async function startServer() {
    ...
    try {
        ...
        config = await (0, config_manager_1.getConfig)('api-admin');
        ...
        tracing = (0, tracing_1.initializeTracing)('guardant-admin-api', {
        ...
        healthChecker = new health_1.HealthChecker('api-admin', '1.0.0');
        metricsCollector = (0, metrics_1.getMetricsCollector)('guardant_admin_api');
        // Update health endpoints with actual implementation
        healthEndpoints = (0, health_1.createHealthEndpoints)(healthChecker);
        // Initialize error handling systems
        errorManager = new error_handling_1.ErrorManager('api-admin', tracing);
        retryManager = (0, retry_1.createRetryManager)('api-admin', tracing);
        circuitBreakerManager = (0, circuit_breaker_1.createCircuitBreakerManager)('api-admin', tracing);
        dlqManager = (0, dead_letter_queue_1.createDLQManager)('api-admin', tracing);
        // Create circuit breakers
        redisCircuitBreaker = circuitBreakerManager.createCircuitBreaker('redis', circuit_breaker_1.CircuitBreakerConfigs.DATABASE);
        rabbitmqCircuitBreaker = circuitBreakerManager.createCircuitBreaker('rabbitmq', circuit_breaker_1.CircuitBreakerConfigs.EXTERNAL_API);
        golemCircuitBreaker = circuitBreakerManager.createCircuitBreaker('golem-l3', circuit_breaker_1.CircuitBreakerConfigs.BLOCKCHAIN);
        ethereumCircuitBreaker = circuitBreakerManager.createCircuitBreaker('ethereum', circuit_breaker_1.CircuitBreakerConfigs.BLOCKCHAIN);
        ...
        authStorage = new index_2.RedisAuthStorage(redis);
        authManager = new index_2.AuthManager(authConfig, authStorage);
        // Initialize authentication middleware now that authManager is ready
        authMiddleware = (0, index_2.createAuthMiddleware)(authManager);
        nestOwnershipMiddleware = (0, index_2.createNestOwnershipMiddleware)('nestId');
        ...
        paymentStorage = new index_3.RedisPaymentStorage(redis);
        // Initialize wallet connector
        walletConnector = (0, index_3.createWalletConnector)({
        ...
        paymentManager = new index_3.PaymentManager(holeskyConfig, paymentStorage, walletConnector);
        ...
        if (config.get('golemEnabled')) {
            try {
                await index_1.golemStorage.initialize();
        ...
        if (rabbitmqConnection && rabbitmqChannel) {
            healthChecker.addCheck('rabbitmq', health_1.commonHealthChecks.rabbitmq(rabbitmqConnection, rabbitmqChannel));
        }
        healthChecker.addCheck('golem-storage', health_1.commonHealthChecks.storage(async () => {
            // Test Golem storage with a simple operation
            await index_1.golemStorage.get('health-check-test');
        }, 'golem-l3'));
        ...
        app.route('/api/admin/platform', platform_routes_simple_1.platformRoutes);
        ...
        if (process.env.NODE_ENV !== 'production' || process.env.ENABLE_DEPLOYMENT_API === 'true') {
            app.route('/api/admin/deployment', deployment_api_1.deploymentApi);
            console.log('🚀 Deployment API enabled at /api/admin/deployment');
        }
        // Mount workers API
        app.route('/api/admin/workers', workers_1.workersApi);
...
process.on('SIGTERM', async () => {
    ...
    await index_1.golemStorage.disconnect();
...
function generateWidgetPreviewHTML(nest, services, options) {
...
if (!global[GLOBAL_SERVER_STATE_KEY]) {
    global[GLOBAL_SERVER_STATE_KEY] = false;
}
...
if (shouldStart) {
    ...
    if (!global[GLOBAL_SERVER_STATE_KEY]) {
        ...
        global[GLOBAL_SERVER_STATE_KEY] = true;
        startServer().catch(error => {
            console.error(`💥 [${startupId}] Server start failed:`, error);
            global[GLOBAL_SERVER_STATE_KEY] = false; // Reset on failure
            process.exit(1);
        });
    }
    else {
